...
/datum/status_effect/grabbed/proc/update_state(datum/source, new_state)
	...
	if(linked && new_state <= GRAB_AGGRESSIVE)
		unlink_mobs()
	if(paired_effect?.hand)
		switch(new_state)
			if(GRAB_PASSIVE)
				paired_effect.hand.color = COLOR_WHITE
			if(GRAB_AGGRESSIVE)
				paired_effect.hand.color = COLOR_BLUE
			if(GRAB_NECK)
				paired_effect.hand.color = COLOR_YELLOW
			if(GRAB_KILL)
				paired_effect.hand.color = COLOR_RED
